...
Queue<Person> findWinners(int numberOfCandies, Queue<Person>guessList) {
	...
	int winam = abs(guessList.peek().getAmount() - numberOfCandies);
	while(!guessList.isEmpty()){
		if(winam > abs(guessList.peek().getAmount() - numberOfCandies)){
			winam = abs(guessList.peek().getAmount() - numberOfCandies);
			winners.clear();
		}
		if(winam == abs(guessList.peek().getAmount() - numberOfCandies)){
